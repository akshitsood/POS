  ...
  const columns = [
    ...
    },
    {
      ...
      render: (id, record) => <div>
        <PlusOutlined className='mx-2' onClick={() => increaseQuantity(record)} />
        <b>{record.quantity}</b>
        <MinusOutlined className='mx-2' onClick={() => decreaseQuantity(record)} />
      </div>
    },
    {
      title: 'Actions',
    ...
    <DefaultLayout>
      ...
      <div className="d-flex justify-content-end flex-column align-items-end">
        ...
        <Button type='primary' onClick={() =>
          setbillchargemodel(true)}>Charge Bill</Button>
      </div>
      <Modal title='Charge Bill' visible={billchargemodel} footer={false} onCancel={() => setbillchargemodel(false)}>
        <Form layout="vertical" onFinish={onFinish}>
          ...
          <Form.Item name='customerPhoneNumber' label="Mobile Number">
            <Input />
          </Form.Item>
